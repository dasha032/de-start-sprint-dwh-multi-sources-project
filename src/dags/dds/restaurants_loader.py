...
class RawRestaurantObj(BaseModel):
    id: int
    object_value: str
    update_ts: datetime
...
class RestaurantsOriginRepository:
    ...
    def list_restaurants(self, restaurant_threshold: int) -> List[RawRestaurantObj]:
        with self._db.client().cursor(row_factory=class_row(RawRestaurantObj)) as cur:
            cur.execute(
                """
...
class RestaurantLoader:
    ...
    def parse_restaurant_data(self, object_value: str, update_ts: datetime):
        """Парсим JSON из object_value и извлекаем данные пользователя"""

        data = str2json(object_value)
        return RestaurantObj(
            restaurant_id=str(data["_id"]),
    ...
    def load_restaurants(self):
        ...
        with self.pg_dest.connection() as conn:
            ...
            for raw_restaurant in load_queue:
                restaurant = self.parse_restaurant_data(
                    raw_restaurant.object_value,
                    raw_restaurant.update_ts
                )
